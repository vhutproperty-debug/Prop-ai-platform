...
import type { DeduplicationResult } from "@/types/firecrawl-import";
...
export async function detectDuplicates(
  ...
  await withDatabase(async () => {
    const [existingBySlug, existingByRera, existingByPair, stagedBySlug] =
      await Promise.all([
        ...
        ImportRecord.findOne({
          slug: project.slug,
          status: { $in: ["staged", "approved", "update"] },
        })
          .select("_id slug")
          .lean(),
      ]);
    ...
    if (
      ...
      String(existingByPair._id) !== String(existingByRera?._id)
    ) {
      matches.push({
...
export function classifyRecord(
  ...
  const publishedMatch = matches.find(
    (m) =>
      m.entityType === "project" &&
      (m.type === "exact_slug" || m.type === "rera_number" || m.type === "builder_project_pair")
  );

  const stagedConflict = matches.find(
    (m) =>
      m.type === "exact_slug" &&
      m.message.includes("already staged")
  );
  ...
  const reraConflict =
    matches.filter((m) => m.type === "rera_number").length > 0 &&
    matches.filter((m) => m.type === "exact_slug").length > 0 &&
    matches.find((m) => m.type === "rera_number")?.existingId !==
      matches.find((m) => m.type === "exact_slug")?.existingId;
  ...
  if (publishedMatch) {
    return {
      recordType: "update",
      matches,
      existingProjectId: publishedMatch.existingId,
      shouldUpdate: true,
    };
  }
...
/** @deprecated Use classifyRecord — kept for legacy pipeline compatibility */
export function hasBlockingDuplicate(matches: DuplicateMatch[]): boolean {
  return classifyRecord(matches).recordType === "conflict";
}

export function resolveRecordStatus(
  recordType: ImportRecordType
): "staged" | "update" | "duplicate" | "conflict" {
  switch (recordType) {
    case "update":
      return "update";
    case "duplicate":
