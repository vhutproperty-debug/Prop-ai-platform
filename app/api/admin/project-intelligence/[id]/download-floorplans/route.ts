...
import { buildFloorPlansZip } from "@/services/project-intelligence/media-download.service";
...
export async function GET(
  ...
  try {
    ...
    if (!report.floorPlans.length) {
      return apiError(new Error("No floor plans available"));
    }

    const buffer = await buildFloorPlansZip(report.floorPlans);
    ...
    return new NextResponse(new Uint8Array(buffer), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${base}-floorplans.zip"`,
      },
    });
  } catch (error) {
